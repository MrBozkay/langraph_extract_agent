...
def process_single_file(
    ...
    try:
        ...
        if not company_info:
            ...
            return {"status": "error", "file": object_name, "error": "No data extracted"}
...
def run_batch_extraction_parallel():
    ...
    md_objects = [obj["object_name"] for obj in objects if obj["object_name"].endswith(".md")]
        ...
        for future in as_completed(future_to_file):
            ...
            try:
                ...
                if result["status"] == "success":
                    ...
                    logger.warning(f"{progress} ❌ {file_name}: {result.get('error', 'Unknown error')}")
